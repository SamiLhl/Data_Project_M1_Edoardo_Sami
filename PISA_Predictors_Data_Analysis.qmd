...
greater national PISA scores, opening the door to policy interventions.
...
geographic or demographic variables may impact students'
...
use the science score to proceed in our analysis. (we can mention the
fact that science is easier to score more as it is easier to prepare)
...
```{r}
Financial_matrix <- Dataset %>% select(Science_Score, GNI.per.capita, GDP.rate) %>% correlate()
...
capita predicting PISA Science score.
...
higher Science Scores (p\<0.001). For every additional dollar, the
Science score increases by 0.001, so that an additional 1000 dollar is
...
```{r}
Demographic_matrix <- Dataset %>% select(Science_Score, Fertility.Rate, Poulation.density, Entrance_Mandatory, Mean_Yrs_Schooling, Survival_Rate_Primary, Gov_Expenditure, Enrolment_Rate, Share_Private) %>% correlate()
...
with PISA scores. (NEED MORE INSIGHTS) We found no link between
government expenditure on education as a share of GDP and education
outcomes.
...
OECD 2018, PISA 2018 Results (Volume V) : Effective Policies, Successful Schools
