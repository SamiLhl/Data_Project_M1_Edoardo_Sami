...
theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
As expected, there seem to be a positive correlation between mean years of schooling.

```{r}
mod_MYrs <- lm(Science_Score ~ Mean_Yrs_Schooling + GNI.per.capita, data=Dataset)
lmMYrs <- Model_to_Table(mod_MYrs)
Table_Entrance <- knitr::kable(lmMYrs)
```
