---
title: "Research Project : Exploratory analysis of potential predictors of PISA Scores"
...
others tackle the way education is provided in the observed
...
governments to act upon in the short term. Finally, this research project provides potential criticism as to what
the PISA score truly reflects. Some variables have been selected to look at
...
## Part 2 : Data sets descriptions


The data exploited throughout this research project comes from prominent international organizations, recognized for providing comprehensive data on many countries around the world. We here quickly describe these organizations and the variables we acquired thanks to them. For further details on how we acquired the data, please consult our Data Treatment document, available in the same [Github
Repository](https://github.com/SamiLhl/Data_Project_M1_Edoardo_Sami).
...
A final remark on the coverage of PISA data relates to China's particular situation. PISA data only cover some regions of China, mostly the coastal, economically growing ones. Unable to acquire information about the average PISA score of the general chinese population, we chose not to include China in our analysis.  
...
moments. The three scores held fairly constant from to 2009 until 2012, after which a major drop took place, observed in 2015. It made the value of each
score drop of around 20 points. In the 2018 observations, the scores decreased more gently giving us our last studied observations, before COVID-19.

The OECD and other researchers have proposed different explanations on the
...
our analysis, we will use the average score, disregarding gender differences, as they even out.

In order to be able to proceed in our analysis and to facilitate the reading of the report we decided to use a mean score between the three categories, which is going to be added as a new variable in our analysis. 
```{r}
...
Model_to_Table <- function(Model){
  ...
  variables <- coefficients_table$term
...
Before diving into any regression model we also include a correlation matrix table to have an overlook and a counterfactual on what to expect from our regressions models.
...
strive towards educating their population to become as productive as in
...
characteristics, if powerful in predicting PISA scores, could be used as
...
associated with one additional point on the PISA score. On its own, GNI per capita explains 20.6% of the variations in PISA scores.

### GDP growth and PISA Scores

Booming economies may have hopes to become an advanced economy in the
span of a generation or two, in a similar fashion that South Korea consolidated after
...
As can be seen, graphical representation of PISA Scores being only
...
At first sight, there seem to be a negative relationship between GDP
...
```{r}
Demographic_matrix <- Dataset |> select (Average_Score, Fertility.Rate, Poulation.density) |> correlate()
...
metrics, such as GNI per capita to ensure that fertility rates truly
...
fertility rate is associated with a 44.28 points decrease in PISA score.
...
## Government policy and other education indicators

```{r}
Dataset_Gov <- Dataset |> rename (Survival_Rate = Survival_Rate_Primary, Entrance_Mand = Entrance_Mandatory, Mean_Yrs_Schl = Mean_Yrs_Schooling )
Policy_matrix_I <- Dataset_Gov |> select(Average_Score, Entrance_Mand, Mean_Yrs_Schl, Survival_Rate) |> correlate()
knitr::kable(Policy_matrix_I, rotate.names = TRUE, angle = 45)
```
```{r}
Policy_matrix_II <- Dataset |> select(Average_Score, Gov_Expenditure, Enrolment_Rate, Share_Private) |> correlate()
knitr::kable(Policy_matrix_II, rotate.names = TRUE, angle = 45)
```
...
Even while controlling for GNI per capita, mandatory entrance age is
here significantly associated with PISA scores (p\<.001). Making entry
to school mandatory one year later is associated with a 14.91 points
...
could lead programs to be differently constructed.
...
As shown above, higher mean years of schooling are associated with higher GNI per capita, and PISA participants are predominantly countries with high mean years of schooling. 
A possible explanation for PISA score difference could be that PISA test are better tailored to school programs imagined for students expected
to continue studying long after the PISA test, and not for programs in which students are expected to leave school soon after. As a result, countries where the population enjoy lower mean years of schooling would be disadvantaged in achieving higher PISA scores.
...
of schooling and average PISA scores. We now wonder whether this positive tendency is due only
...
increases of 6.12 in the score.
...
effect, therefore we can conclude that higher survival rate to the last grade
of primary education is beneficial to higher PISA score
A model using GNI per capita and Survival Rate can explain 50% of the variance in PISA scores.  
...
ylab("Average_Score") +
ggtitle("Scatterplot of PISA score given government expenditure") +
theme_minimal()+
theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
government expenditure of 1% of the national budget.By increasing the
...
However we wanted to test the effect and controlling the effect on
regression, which demonstrate that the results are not significant (p >0.1)

### Enrollment rate

The enrolment rate is a variable that measure the percentage of pupils
enrolled in any educational program. In the world the enrolment rate
...
The model shows that there is a positive correlation between the
variables, and that the enrolment rate is a significant predictor of
PISA scores (p<.001). The best linear model provide sizeable coefficient.
For an increase in the enrolment rate of 1, the expected national PISA
score increases of 5.92 in the score. However, a positive relationship between economic growth and enrolment rate exists(Gumus, S. and Kayhan, S. 2012) therefore in order to
...
significant results and even if the effect of the enrolment rate is
...
From the results we can conclude that countries with a greater share of students in private schools tend to score lower than countries with less students in private schools. The OECD reported on the matter (OECD, 2018) stating that if accounting for socio-economic background, students in public schools score higher in every category. As such, in similar countries, with varying share of students in private schools, we may indeed expect the countries with the highest share of public school students to enjoy a higher national PISA score.
...
Now accounting for the GNI per capita, we are able to confirm the trend of countries in which more students attend private schools usually perform more poorly than their counterparts with similar economic conditions.
...
In the final regression model, all remaining variables are statistically significant (p<.01), and the model is able to explain almost 75% of the variance in PISA scores.

## Part 4 : Conclusion and Limitations
...
capita was an important predictor, while GDP growth does not appear relevant or negatively impact educational attainments. 
While the first observation is well known and recognized, the latter may seem counter-intuitive.
These results could possibly be related to the time of observation and the observed countries. Depending on the level of economic advancement of a country it may
prioritize low skill jobs rather than targeting high value added sectors, while the contrary would occur past some level of economic advancement. 
...
Finally, we looked at how education policy affect PISA scores. We have found that enrolment rates and survival rates were significantly and positively associated with PISA scores, even while controlling for GNI per capita. On the other hand, the share of private school enrolment was significantly and negatively associated with PISA scores. The literature have supported our results stating that on average students enrolled in private schools, tend to score lower than those in public schools. We found no link between government expenditure on education as a share of GDP and education outcomes.

While a linear model built out of the studied variables allows to explain 70% of the variance in PISA scores, it is important to acknowledge that some of the variables such as GNI per capita or Entrance age were not in fact behaving linearly. Moreover, most of the developing world did not participate into PISA, meaning we lack important observations to assess best what impacts education outcomes.
Further studies would benefit from developing economies joining the ranks PISA participants. It would increase the range of observed values taken by variables and the number of independent observations, making the use of four observations of the same country at different times unnecessary. 

## Part 5 : Annex + Sources


[Link to the report's Github
Repository](https://github.com/SamiLhl/Data_Project_M1_Edoardo_Sami). 

### Annex 

  Throughout this report, we made extensive use of a dataset previously constructed out of smaller UNESCO, World Bank and OECD datasets. After formatting them into similar formats, with comprehensible and common variable names, we joined all the data together and got rid of redundant and unusable data. Among the actions necessary for the completion of this task, we had to rename variables and set the year associated to each data points, rid ourselves of the standard error of PISA scores, deemed irrelevant to our research.
  The links to access the different variables' datasets we used throughout the treatment phase are as such: 
...
- [UNESCO Interface](http://data.uis.unesco.org/)

- [PISA Data Explorer](https://pisadataexplorer.oecd.org/ide/idepisa/) 
  
  For further details, the Data treatment .qmd file is accessible in the same [Github
Repository](https://github.com/SamiLhl/Data_Project_M1_Edoardo_Sami).
  
### Sources

Barshay, J. (2023), Proof points there is a worldwide problem in math and it’s not just about the pandemic. The Hechinger Report.
...
Gumus, S. and Kayhan, S. (2012), The Relationship between Economic Growth and School Enrollment Rates: Time SeriesEvidence from Turkey, Educational Policy Analysis and Strategic Research, 201

Hu, H. (2023) The Consequences of Fertility Decline on Educational Attainment in China. Popul Res Policy Rev 42, 94. https://doi-org.proxy.bu.dauphine.fr/10.1007/s11113-023-09834-7
...
OECD 2018, PISA 2018 Results (Volume V) : Effective Policies, Successful Schools, OECD publications 

Osili, U. and Long, B. T. (2007). Does female schooling reduce fertility? evidence from nigeria.. https://doi.org/10.3386/w13070

Sittisom,W. (2020). Barriers in way towards quality education: poverty, fertility rate and increasing population, Journal of Security and Sustainability Issues, 9(January), 215-227. http://doi.org/10.9770/jssi.2020.9.J(16)
