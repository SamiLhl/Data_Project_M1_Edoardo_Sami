...
Household income
```{r}
Household_income <- vroom(here("HI_Data/NAAG_27112023180449110.csv"))
```
...
Household_income2 <- Household_income |> select(!where(is_na)) 
...
Household_income3 <- Household_income3 |> pivot_wider(names_from = Indicator, 
              values_from = Value,
              names_sep = "_") |> rename(
    Real_Household_Disp_Income_adj = "Real household net adjusted disposable income, deflated by actual individual consumption, millions of national currency",
    Real_Household_Disp_Income = "Real household net disposable income, deflated by household Final consumption, millions of national currency"
  ) 
#Household_income3 <- Household_income3 |> select(-'2000':-'2008')
