...
```
All the UNESCO Variables are treated so that all that remains is the value of interest for each data as well as the year of observation and the country so we can join them.
