...
We used pivot longer to transform the data and make them more accessible and ready for the merge. We then adjusted the variables name, and removed superfluous observations. 
...
GNI per capita: Measure of the total income earned by a country's residents, plus, the net inflow of primary income (comprising compensation received by employees and income from property) from abroad.
...
PISA <- TAG_Name |> left_join(PISA, by = "Jurisdiction") |> left_join(Add_Tag, by = "Jurisdiction") 
