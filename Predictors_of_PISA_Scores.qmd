...
### Data on GDP 
```{r}
GDP_growth <- vroom(here("GDP_data/GDP-data.csv"))
```
...
- The data is sometimes way too massive compared to what we want to study, so some of the treatment will need to be done separately for each variables.
...
Country_Code_test <- Gov_Exp |> distinct(LOCATION)
...
Start_Yr <- UNESCO_Treatment(Start_End_Yr) |> filter(Indicator == "Start month of the academic school year (pre-primary to post-secondary non-tertiary education)")
End_Yr <- UNESCO_Treatment(Start_End_Yr) |> filter(Indicator == "End month of the academic school year (pre-primary to post-secondary non-tertiary education)")
