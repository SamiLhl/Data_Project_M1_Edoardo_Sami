...
Household income
```{r}
Household_income <- vroom(here("HI_Data/NAAG_28112023103146628.csv"))
```
...
Pisa_score_readingscale <- vroom(here("Pisa_Score/Readingscale - Copie.csv"))
Pisa_score_readingscale_gender <- vroom(here("Pisa_Score/Readingscale_gender - Copie.csv"))
Pisa_score_Mathscale <- vroom(here("Pisa_Score/Mathscale - Copie.csv"))
Pisa_score_Mathscale_gender <- vroom(here("Pisa_Score/Mathscale_gender - Copie.csv"))
Pisa_score_Sciencescale <- vroom(here("Pisa_Score/Sciencescale - Copie.csv"))
Pisa_score_Sciencescale_gender <- vroom(here("Pisa_Score/Sciencescale_gender - Copie.csv"))
```
...
```{r Treating UNESCO Data}
Gov_Exp <- UNESCO_Treatment(Gov_Exp)
Gov_Exp <- Gov_Exp |> filter(Indicator == "Government expenditure on lower secondary education as a percentage of GDP (%)")

Enrolment <- UNESCO_Treatment(Enrolment)
Enrolment <- Enrolment |> filter(Indicator == "Total net enrolment rate, lower secondary, both sexes (%)")

MeanSchooling <- UNESCO_Treatment(MeanSchooling)
MeanSchooling <- MeanSchooling |> filter(Indicator == "Mean years of schooling (ISCED 1 or higher), population 25+ years, both sexes")

Share_Studs_Priv <- UNESCO_Treatment(Share_Studs_Priv)
Share_Studs_Priv <- Share_Studs_Priv |> filter(Indicator =="Percentage of enrolment in lower secondary education in private institutions (%)")

Survival_Rate <- UNESCO_Treatment(Survival_Rate)
Survival_Rate <- Survival_Rate |> filter(Indicator == "Survival rate to the last grade of primary education, both sexes (%)")

EntranceAge <- UNESCO_Treatment(EntranceAge)
EntranceAgeEarly <- EntranceAge |> filter(Indicator == "Official entrance age to early childhood education (years)")
EntranceAgeManda <- EntranceAge |> filter(Indicator == "Official entrance age to compulsory education (years)")

Start_End_Yr <- UNESCO_Treatment(Start_End_Yr)
End_Yr <- Start_End_Yr |> filter(Indicator == "End month of the academic school year (pre-primary to post-secondary non-tertiary education)")
Start_Yr <- Start_End_Yr |> filter(Indicator == "Start month of the academic school year (pre-primary to post-secondary non-tertiary education)")
...
```{r}
FertiliT <- Fertility_Rate |> select(Country_Name, Country_Code, Indicator_Name, X2009, X2012, X2015, X2018) |> rename( "2009" = "X2009", "2012"="X2012", "2015"="X2015", "2018"="X2018")
DensiT <- Pop_Density |> select(Country_Name, Country_Code, Indicator_Name, Y2009, Y2012, Y2015, Y2018) |> rename( "2009" ="Y2009", "2012"="Y2012", "2015"="Y2015", "2018"="Y2018")
```
...
Pisa_score_Mathscale_gender <- Pisa_score_Mathscale_gender |> rename("Math_Score_F" = `Average(Female)`, "Math_Score_M" = `Average(Male)`) |> fill(`Year/Study`)|>select(-`Standard Error M`, -`Standard Error F`)
...
Pisa_score_readingscale_gender <- Pisa_score_readingscale_gender |> rename("Reading_Score_F" = `Average(Female)`,"Reading_Score_M" = `Average(Male)`) |> fill(`Year/Study`) |>select(-`Standard Error M`, -`Standard Error F`)
...
Pisa_score_Sciencescale_gender <- Pisa_score_Sciencescale_gender |> rename("Science_Score_F" = `Average(Female)`, "Science_Score_M" = `Average(Male)`) |> fill(`Year/Study`)|>select(-`Standard Error M`, -`Standard Error F`)
...
there is still some cleaning to do, as some of the data (malasya 2015) are to be counted
```{r}
...
```{r}
is_na <- function(my_var) {
  my_var |> is.na() |> all()
}
Household_income3 <- Household_income |>
  select(!where(is_na)) |>
  select(-`PowerCode Code`, -PowerCode, -`Flag Codes`, -Flags, -Time)
Household_income3 <- Household_income3 |> pivot_wider(names_from = Indicator, 
              values_from = Value,
              names_sep = "_") |> 
  rename(Real_Household_Disp_Income = "Real household net disposable income, deflated by household Final consumption, millions of national currency"
  ) 
Household_income3 <- Household_income3 %>%
  filter(TIME < 2000 | TIME > 2009)
Household_income3 <- Household_income3 |>
  pivot_wider(names_from = TIME,
              values_from = Real_Household_Disp_Income) 
data(mtcars)

```

Small example on how to present the dataset for the project.
```{r}
variable_names <- names(Household_income3)
observations_count <- sapply(Household_income3, function(x) length(x))
variable_table <- data.frame(Variable = variable_names, Observations = observations_count)
knitr::kable(variable_table, caption = "Number of Observations for Each Variable")
```
